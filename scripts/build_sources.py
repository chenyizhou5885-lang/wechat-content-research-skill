...
def forced_c_reason(item, final_url):
    ...
    path = urlsplit(final_url).path.lower() if valid_url(final_url) else ""
...
def audit_result(item):
    ...
    if item.get("platform") == "wechat":
        if item.get("verification") != "verified":
            reasons.append("WeChat article was not verified by browser")
        if not all(item.get(key) for key in ("verified_title", "verified_publisher", "verified_published_at")):
            reasons.append("missing verified WeChat title/publisher/date")
...
def build(raw):
    ...
    for claim in raw.get("claim_ledger", []):
        ...
        claim_audit.append({
            **claim,
            ...
            "grades": grades,
            "passed": passed,
            "reason": ("covered by primary/independent evidence" if passed else
    ...
    wechat_required = requirements.get("wechat_research_required", False) is True
    ...
    wechat_ready = (not wechat_required or
                    (len(verified_wechat) >= min_articles and
                     len(independent_publishers(verified_wechat)) >= min_publishers))

    min_candidates = int(requirements.get("min_candidates", 12))
    min_visited = int(requirements.get("min_visited_results", 8))
    min_accepted = int(requirements.get("min_accepted_sources", 5))
    min_channels = int(requirements.get("min_discovery_channels", 2))
...
def main():
    ...
    result = build(read_json(args.raw))
